  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white relative">
      ...
      <LeaderboardDialog
        ...
        subtitle={`${liveSubmissions.length} participants • ${liveSubmissions.reduce((sum, s) => sum + s.vote_count, 0)} votes`}
        entries={liveSubmissions.map((s) => ({
          id: s.id,
          name: s.name,
          subtitle: s.blog_title,
          vote_count: s.vote_count,
        }))}
        onView={(entry) => {
          const found = liveSubmissions.find((s) => s.id === entry.id);
          if (found) setSelectedLiveSubmission(found);
